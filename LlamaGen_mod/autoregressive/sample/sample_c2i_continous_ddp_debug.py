...
from LlamaGen_mod.autoregressive.models.generate import generate, generate_code_wo_kvcache, generate_continous_code, generate_debug
...
def x_to_mem(x, model):
    ...
    mem = model.fvae.get_quant_from_vae_latent(x_reshape)  # (B, z_dim, latent_size, latent_size) -> (B, zq_dim, latent_size, latent_size)
...
def main(args):
    ...
    vis_dataset = Subset(dataset, list(range(n)))
    vis_loader = DataLoader(vis_dataset, batch_size=n, shuffle=False, pin_memory=True)
    # ------------- #
    ...
    for _ in pbar:
        ...
        for x, y in vis_loader:
            ...
            gen_ar_codes, gen_gt_codes = generate_code_wo_kvcache(
                model=gpt_reg_model, cond=y.long(), gt_codes=x, mem=mem, seq_len=256,
                emb_masks=None, cfg_scale=args.cfg_scale, cfg_interval=args.cfg_interval
            )

            #code_sample_reshaped = code_gen_context.reshape(n, latent_size, latent_size, -1).permute(0, 3, 1, 2)  # (b, 16, 16, 16)
            gen_ar_img = gpt_reg_model.fvae.kl.decode(gen_ar_codes.reshape(n, latent_size, latent_size, -1).permute(0, 3, 1, 2))
            gen_gt_img = gpt_reg_model.fvae.kl.decode(gen_gt_codes.reshape(n, latent_size, latent_size, -1).permute(0, 3, 1, 2))
            ...
            # ------------------------------------------------------------------- #
            # code_sample_gt_context, code_sample_gen_context = generate_debug(
            #     model=gpt_reg_model, cond=y, gt_codes=x, mem=mem, max_new_codes=latent_size ** 2,
            #     cfg_scale=args.cfg_scale, cfg_interval=args.cfg_interval
            # )

            # code_sample_gt_context = code_sample_gt_context.reshape(n, latent_size, latent_size, -1).permute(0, 3, 1, 2)  # (b, 16, 16, 16)
            # img_sample_gt_context = fvae.kl.decode(code_sample_gt_context)  # (b, 3, 256, 256)

            # code_sample_gen_context = code_sample_gen_context.reshape(n, latent_size, latent_size, -1).permute(0, 3, 1, 2)  # (b, 16, 16, 16)
            # img_sample_gen_context = fvae.kl.decode(code_sample_gen_context)  # (b, 3, 256, 256)
            ...
            #     index = i * dist.get_world_size() + rank + total
            #     if index < args.num_fid_samples:
            #         save_image(img_sample_gt_context[i], f"{sample_folder_dir}/{index:06d}_gt_context.png", normalize=True, value_range=(-1, 1))
            #         save_image(img_sample_gen_context[i], f"{sample_folder_dir}/{index:06d}_gen_context.png", normalize=True, value_range=(-1, 1))
            #         save_image(gt_code_dec[i], f"{sample_folder_dir}/{index:06d}_gt.png", normalize=True, value_range=(-1, 1))
            #         save_image(mem_code_dec[i], f"{sample_folder_dir}/{index:06d}_mem.png", normalize=True, value_range=(-1, 1))
            #     else:
            #         break
...
if __name__ == "__main__":
    ...
    parser.add_argument("--info", type=str, default="", help="additional info for the run")
    parser.add_argument("--dataset", type=str, default='cifar10_code')
    parser.add_argument("--train-code-path", type=str, required=True)
    parser.add_argument("--val-code-path", type=str, required=True)
