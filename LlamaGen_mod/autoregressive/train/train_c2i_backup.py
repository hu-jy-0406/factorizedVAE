...
def main(args):
    ...
    if args.gpt_ckpt:
        ...
        if "steps" in checkpoint:
            ...
            filename = args.gpt_ckpt.split('/')[-1].split('.')[0]
            train_steps = int(''.join(filter(str.isdigit, filename))) if any(char.isdigit() for char in filename) else 0
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        for x, y in loader:
            x = x.to(device, non_blocking=True)#x.shape = ([60, 1, 2, 256]) 其中2表示数据增强的个数，下面直接reshape会导致序列长度变化，需要注意
            y = y.to(device, non_blocking=True)#y.shape = ([60, 1])
            z_indices = x.reshape(x.shape[0], -1)# ([60, 512])
            c_indices = y.reshape(-1)# ([60])
            assert z_indices.shape[0] == c_indices.shape[0]
            with torch.cuda.amp.autocast(dtype=ptdtype):  
                _, loss = model(cond_idx=c_indices, idx=z_indices[:,:-1], targets=z_indices)
            #c_indices.shape = ([60]),z_indices.shape = ([60, 512]), idx.shape = ([60, 511])
