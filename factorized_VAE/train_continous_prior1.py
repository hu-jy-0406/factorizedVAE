...
class ContinousPrior(nn.Module):
    ...
    def forward(self, x):
        ...
        latents_i = predictions[0].reshape(8, 8, C)
        ...
        if len(samples) > 0:
            grid_img = Image.new('RGB', (32 * len(samples), 32))
            for i, img in enumerate(samples):
                grid_img.paste(img, (i * 32, 0))
            grid_img.save("factorized_VAE/ContinousPrior/dec_epoch.png")

        gt_dec_detach = gt_dec.to(device).detach()  # Detach the ground truth image for perceptual loss
...
def train(model, discrete_prior, resume_path=None):
    ...
    for epoch in range(start_epoch, epochs):
        ...
        if (epoch+1) % 500 == 0 or (epoch+1) == epochs:
            ...
            ckpt_path = f"factorized_VAE/ContinousPrior/ContinousPrior_single_disweight_nomem_epoch{epoch+1}.pth"
